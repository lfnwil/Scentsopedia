...
export async function getAllFragrances(req, res, next) {
  try {
    ...
    res.json(filterFragrances(fragrances, req.query));
  } catch (error) {
    next(error);
  }
}
...
export async function getFragranceById(req, res, next) {
  try {
    res.json(await findFragranceOrFail(req.params.id));
  } catch (error) {
    next(error);
  }
}
...
export async function createFragrance(req, res, next) {
  try {
    const fragrance = await Fragrance.create(await buildFragrancePayload(req.body));
    res.status(201).json(await findFragranceOrFail(fragrance.id));
  } catch (error) {
    next(error);
  }
}
...
export async function updateFragrance(req, res, next) {
  try {
    const fragrance = await findFragranceOrFail(req.params.id);
    await fragrance.update(await buildFragrancePayload(req.body, {
      partial: true,
      currentFragrance: fragrance,
    }));
    res.json(await findFragranceOrFail(fragrance.id));
  } catch (error) {
    next(error);
  }
}
